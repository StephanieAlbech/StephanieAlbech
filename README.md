
<div align="center"># ✦ Olá! Eu sou a Stephanie ✦

### 💻 Desenvolvedora Front-end em formação

<p>
  <i>Transformando aprendizado em projetos e projetos em experiência.</i>
</p>

<br>

<a href="https://github.com/StephanieAlbech">
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white">
</a>

</div>

---

## 🌷 Sobre mim

Sou estudante de tecnologia e desenvolvedora **Front-end em formação**, atualmente focada em aprimorar minhas habilidades e construir projetos cada vez mais completos.

Meu objetivo é evoluir para o desenvolvimento **Full-Stack**, unindo desenvolvimento de interfaces, lógica de programação, back-end e bancos de dados.

🎯 **Objetivo:** conquistar minha primeira oportunidade na área de desenvolvimento e continuar evoluindo profissionalmente.

🌱 **Atualmente:** estudando e colocando em prática novos projetos.

---

## 🛠️ Tecnologias

<div align="center">

<img src="https://skillicons.dev/icons?i=html,css,js,git,github" />

</div>

### 📚 Atualmente estudando

<div align="center">

<img src="https://skillicons.dev/icons?i=nodejs,sql,c" />

</div>

---

## 🚀 Projetos

Aqui estão alguns dos projetos que fazem parte da minha jornada de aprendizado:

| Projeto                   | Descrição                                                      |
| ------------------------- | -------------------------------------------------------------- |
| 🌐 **Projeto Social**     | Página web desenvolvida para praticar HTML e CSS               |
| 📱 **Projeto Android**     | Landing page desenvolvida com HTML e CSS                       |
| 📖 **Projeto Cordel**     | Projeto focado em estrutura HTML, estilização e responsividade |
| 🎨 **Projetos Front-end** | Exercícios e projetos desenvolvidos durante meus estudos       |

> 💡 Novos projetos próprios serão adicionados conforme avanço nos meus estudos.

---

## 📈 Minha jornada

```text
HTML & CSS
    │
    ▼
JavaScript
    │
    ▼
Projetos Front-end
    │
    ▼
Git & GitHub
    │
    ▼
Back-end
    │
    ▼
Banco de Dados
    │
    ▼
🚀 Full-Stack
```

---

## 🌱 O que estou buscando aprender

* ⚡ JavaScript
* 🟢 Node.js
* 🗄️ Bancos de dados e SQL
* 🔌 APIs
* 🔐 Autenticação
* 🌐 Desenvolvimento Full-Stack
* 📦 Boas práticas de Git e GitHub
* 🧩 Desenvolvimento de projetos completos

---

## 💜 Vamos nos conectar?

<div align="center">

<a href="https://github.com/StephanieAlbech">
  <img src="https://img.shields.io/badge/GitHub-StephanieAlbech-181717?style=for-the-badge&logo=github">
</a>

<a href="https://www.linkedin.com/in/stephanie-frança-de-albeche-a44680349/">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white">
</a>


</div>

<br>

<div align="center">

### ✨ Sempre aprendendo. Sempre construindo. ✨

</div>
